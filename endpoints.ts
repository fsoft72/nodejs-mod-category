...
export const init = ( liwe: ILiWE ) => {
	...
	app.post( "/api/category/admin/add", perms( [ "category.editor" ] ), ( req: ILRequest, res: ILResponse ) => {
		const { title, slug, id_parent, description, modules, top, visible, image, ___errors } = typed_dict( req.fields || req.body, [
	...
	app.patch( "/api/category/admin/update", perms( [ "category.editor" ] ), ( req: ILRequest, res: ILResponse ) => {
		const { id, id_parent, title, slug, description, modules, top, visible, image, ___errors } = typed_dict( req.fields || req.body, [
